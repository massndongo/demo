...
public class UtilisateurController {
    ...
    @GetMapping(path="/findByFirstname/{firstname}")
    public ResponseEntity<List<Utilisateur>> getUserByFirstnameAndLastname(@PathVariable String firstname, @PathVariable String firstnam) {
        List<Utilisateur> utilisateurs = userService.findByFirstnameAndLastname(firstname);
        if (utilisateurs.isEmpty()) {
            return new ResponseEntity<List<Utilisateur>>(HttpStatus.NO_CONTENT);
        } else {
            return new ResponseEntity<List<Utilisateur>>(utilisateurs, HttpStatus.OK);
